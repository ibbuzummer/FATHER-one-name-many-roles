...
function CreateDOMElements(mode){
    ...
    switch(mode){
        case "start":
            let startBtn = createButton('get ready for some fun on fathers day');
            let journalBtn = createButton('play these games to unlock messages');
            let minigameBtn = createButton('Get Set Go!');
            ...
            startBtn.elt.disabled = true;
            journalBtn.elt.disabled = true;
        ...
        case "journal":
            ...
            break;
        case "MiniGame":
            let button1 = createButton('messages await you');
            let button2 = createButton('press the PLAY button to start');
...
function draw(){
    ...
    if(gameState === "start"){
        ...
        textFont("Trebuchet MS");
        textStyle(BOLD);
        text("Wishing you a Happy Father's Day Dad!",width/2,50);
        text("Play these minigames for some special",width/2,100)
        text("MESSAGES",width/2,150)
        textSize(2);
        textAlign(LEFT);
        fill(0,0,0);
