...
function game1(){
    ...
    if(minigamestate == "playing"){
        ...
        textAlign(CENTER);
        fill(0,255,255,230);
        text("Help the elders GET A CAR RIDE!",width/2,100);
        textSize(20);
        text("Use the mouse to drag them to a on coming car",width/2,125);
        pop();
        let cars = [
    ...
    if(minigamestate == "over"){
        ...
        text("I always love the way you help older people and our grandparents",width/2,150);
        text("In all the work Thank you DAD for everything you do for us ",width/2,200);
        text("Wishing you a very happy FATHERS DAY",width/2,225);
        pop();
...
function game2(){
    ...
    if(minigamestate == "playing"){
        ...
        textSize(20);
        textAlign(CENTER);
        text("get the germs inside the box!",width/2,100);
        text("Drag them to the box",width/2,125);
        pop();
        if(frameCount%150==0){
    ...
    if(minigamestate == "over"){
        ...
        textSize(25);
        fill(0,0,0);
        text("Just like in the game you put the germs in the box",400,100);
        text("so they dont get to our home",400,125);
        text("you protect us from having to go out in the filth and keep us safe",400,150);
        if(keyIsPressed && key == 'n'){game
            game++;
            timer = 30;
            minigamestate = "playing";
        }
...
function game3(){
    ...
    if(minigamestate === "playing"){
        ...
        text("just like you catch these thieves in the game",width/2,150);
        text("you also protect our house from threats",width/2,175);
        pop();
        if(frameCount%60==0){
